        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-600">
                    {t('Welcome back! Here\'s what\'s happening with your Vietnamese learning platform.')}
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl font-bold text-gray-900">                {formLesson.length > 0 ? formLesson[0].totalUser?.toLocaleString() : '---'}
                            </p>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-gray-600">{t('Total Lessons')}</p>
                            <p className="text-3xl font-bold text-gray-900">{data?.lesson.length}</p>
                            {/* <div className="flex items-center mt-2">
                                <FontAwesomeIcon icon={faArrowUp} className="text-green-500 text-sm mr-1" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-semibold text-gray-900">{t('Recent Activities')}</h2>
                    ...
                    <span class="text-blue-600 font-semibold text-sm">
                        ${activity.fullName ? activity.fullName.split(' ').map(n => n[0]).join('') : '?'}
                    </span>
                `;
                                            }}
                                        />
                                ...
                                <div className="flex-1">
                                    <p className="text-sm font-medium text-gray-900">
                                        <span className="font-semibold">{activity.fullName}</span> {activity.action.toLowerCase()}
                                        {activity.title && <span className="text-blue-600"> "{activity.title}"</span>}
                                        {activity.typeGame && <span className="text-purple-600"> ({activity.typeGame})</span>}
                                    </p>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-semibold text-gray-900">{t('Top Lessons')}</h2>
                            ...
                            <div key={lesson.id} className="flex items-center space-x-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex items-center space-x-3 mt-1">
                                        <span className="text-xs text-gray-500">{lesson.countCompleted} completions</span>
                                        <div className="flex items-center">
                                            <span className="text-yellow-400 text-xs">★</span>
                                            <span className="text-xs text-gray-500 ml-1">{start(lesson.countCompleted)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
